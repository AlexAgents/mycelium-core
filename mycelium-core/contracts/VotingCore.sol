...
contract VotingCore {
    ...
    // dedicated events for stage transitions
    event VotingStarted(uint256 blockNumber);
    event VotingFinished(uint256 blockNumber);
        ...
        onlyStage(Stage.Setup)
    {
        ...
        emit VotingStarted(block.number);
    }
        ...
        onlyStage(Stage.Active)
    {
        ...
        emit StageChanged(uint8(stage));
        emit VotingFinished(block.number);
    }
