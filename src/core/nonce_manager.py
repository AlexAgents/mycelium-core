...
class NonceManager:
    ...
    def get_next_nonce(self) -> int:
        ...
        Первый вызов синхронизируется с сетью.
        Последующие вызовы инкрементируют локальный счётчик.
        """
        with self._lock:
            if self._nonce is None:
                self._nonce = self._fetch_network_nonce()
                logger.debug(
                    "Nonce initialized for %s: %d", self._address, self._nonce
                )
            else:
                self._nonce += 1

            logger.debug("Nonce %d issued for %s", self._nonce, self._address)
            return self._nonce
